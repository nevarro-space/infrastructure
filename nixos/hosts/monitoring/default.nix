{ config, lib, terraform-outputs, ... }: with lib; let
  ...
  services.prometheus = {
    ...
    scrapeConfigs = (
    ...
    ) ++ [
      {
        ...
        static_configs = [
          {
            targets = [ "${terraform-outputs.matrix_server_internal_ip.value}:9009" ];
            labels = { instance = matrixDomain; job = "master"; index = "1"; };
          }
          {
            # Federation sender 1
            targets = [ "${terraform-outputs.matrix_server_internal_ip.value}:9101" ];
            labels = { instance = matrixDomain; job = "federation_sender"; index = "1"; };
          }
          {
            # Federation sender 2
            targets = [ "${terraform-outputs.matrix_server_internal_ip.value}:9106" ];
            labels = { instance = matrixDomain; job = "federation_sender"; index = "2"; };
          }
          {
            # Federation reader 1
            targets = [ "${terraform-outputs.matrix_server_internal_ip.value}:9102" ];
            labels = { instance = matrixDomain; job = "federation_reader"; index = "1"; };
          }
          {
            # Event persister 1
            targets = [ "${terraform-outputs.matrix_server_internal_ip.value}:9103" ];
            labels = { instance = matrixDomain; job = "event_persister"; index = "1"; };
          }
          {
            # Event persister 2
            targets = [ "${terraform-outputs.matrix_server_internal_ip.value}:9107" ];
            labels = { instance = matrixDomain; job = "event_persister"; index = "2"; };
          }
          {
            # Synchotron 1
            targets = [ "${terraform-outputs.matrix_server_internal_ip.value}:9104" ];
            labels = { instance = matrixDomain; job = "synchotron"; index = "1"; };
          }
          {
            # Media repo 1
            targets = [ "${terraform-outputs.matrix_server_internal_ip.value}:9105" ];
