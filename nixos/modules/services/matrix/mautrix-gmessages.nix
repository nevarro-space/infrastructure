...
in {
  options.services.mautrix-gmessages = {
    ...
    settings = lib.mkOption {
      ...
      description = ''
        {file}`config.yaml` configuration as a Nix attribute set. See
        https://docs.mau.fi/bridges/go/setup.html?bridge=gmessages for details
        about what needs to go into the configuration for the bridge.
      '';
      example = {
        appservice = {
          database = {
